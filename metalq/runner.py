...
def _execute(circuit: Circuit) -> int:
    """Allocate a simulator handle, apply all gates, return handle."""
    h = _ffi.create(circuit.n_qubits)
    for op in circuit.ops:
        _ffi.apply_gate(h, op.gate_id, op.target, op.control,
                        op.resolved_theta(), op.phi, op.lam)
    return h

def run(circuit: Circuit, shots: int = 1024,
        backend: str = "mps") -> RunResult:
    h = _execute(circuit)
    ...
    return RunResult(counts=dict(counts), shots=shots,
...
def expect(circuit: Circuit, hamiltonian) -> float:
    ...
    result  = _ffi._lib.mq_expect(
        h,
        pauli_ids.ctypes.data_as(__import__('ctypes').c_void_p),
        qubits.ctypes.data_as(__import__('ctypes').c_void_p),
        coeffs.ctypes.data_as(__import__('ctypes').c_void_p),
        n_terms, n_qpt,
    )
    _ffi.destroy(h)
    return float(result)